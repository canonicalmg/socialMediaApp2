...
def home(request):
    if request.user.is_authenticated():
        ...
        for j in userWallPosts:
            ...
            sendThesePosts.append([j.postSender.username, j.content, j.created_at.strftime("%I:%M %B %d, %Y"), j.postReceiver.username, profilePrimary.profilePic.picLocation.url, postComments, j.pk])
...
def newUserSignUp(request):
    ...
    if request.is_ajax():
        if request.method == "POST":
            ...
            newUser = User.objects.create_user(str(data[2].split('@', 1)[0]), str(data[2]), str(data[3]))
            newUser.first_name = str(data[0])
            newUser.last_name = str(data[1])
            newUser.save()

            userName = str(data[2].split('@', 1)[0])
            #create profile
...
def headerSignIn(request):
    ...
    if request.is_ajax():
        if request.method == "POST":
            data = request.POST.getlist("data[]")
            user = authenticate(username=str(data[0].split('@', 1)[0]), password=str(data[1]))
            if user is not None:
                if user.is_active:
                    login(request, user)
                    return HttpResponse("return this string")
    ...
    if User.objects.filter(username=string).exists():
        ...
        try:
            ...
            for j in userWallPosts:
                ...
                postComments = postComment.objects.filter(post=j)
                sendThesePosts.append([j.postSender, j.content, j.created_at.strftime("%I:%M %B %d, %Y"),sendingProfile.getPrimaryPicURL(), postComments, j.pk ])
...
def searchUsers(request):
    if request.is_ajax():
        if request.method == "POST":
            data = request.POST.get("data")
            selectedUser = User.objects.filter(username__contains=data)
            #return HttpResponse(selectedUser.first().first_name)
            userArray = []
